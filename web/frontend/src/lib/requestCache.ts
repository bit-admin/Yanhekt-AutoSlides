 ...
 *
 * Every browser request here is a Worker invocation (and a Yanhekt round-trip
 * behind it), so a repeat click that fires a second identical GET is pure waste:
 * the Worker sets `Cache-Control: no-store` on everything it proxies, and
 * nothing downstream absorbs it. This module is the one place that does.
 *
