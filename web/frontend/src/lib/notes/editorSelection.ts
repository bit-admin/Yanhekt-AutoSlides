 ...
 * at runtime. That ice-blue stacks on top of `.ce-block--selected` and reads
 * as two highlight layers. Rewrite the injected sheet and pin one opaque
 * color for both ::selection and block-select (same approach as desktop).
 */
...
function rewriteSheet(sheet: CSSStyleSheet, inline: string, block: string, fg: string): void {
  const walk = (rules: CSSRuleList): void => {
    for (const rule of Array.from(rules)) {
      ...
      if (sel.includes('::selection') || sel.includes('::-moz-selection')) {
        ...
        rule.style.setProperty('color', fg, 'important');
      }
...
export function syncEditorJsSelectionStyles(): void {
  ...
  for (const sheet of Array.from(document.styleSheets)) {
    try {
      rewriteSheet(sheet, inline, block, fg);
    } catch {
  ...
  if (!listening) {
    ...
    new MutationObserver(() => syncEditorJsSelectionStyles()).observe(document.documentElement, {
      attributes: true,
