...
│       │   │
│       │   ├── results/                      # 结果视图
│       │   │   ├── useResultsView.ts         # 主 composable (~932 行)
...
| **results** | `features/results/` | 结果视图数据加载、去重、裁剪 | `useResultsView`, `resultsDataLoader`, `resultsDedupPipeline`, `resultsCropPipeline` |
...
| **course** | `features/course/` | 课程列表、课时页面 | `useCourseList`, `useSessionPage` |
| **settings** | `features/settings/` | 设置管理 | `useSettings`, `useAdvancedSettings`, `useGeneralSettings` 等 |
| **platform** | `features/platform/` | 认证、缓存、教程 | `useAuth`, `useCacheManagement`, `useTour` |
...
- **异步写入**：通过 `window.electronAPI.config.*` 设置器
...
│  Panel   │  ┌─────────────────────────┐     │    Panel      │
│          │  │  CoursePage /           │     │               │
│ Auth     │  │  SessionPage /          │     │ Download      │
│ Settings │  │  PlaybackPage           │     │ Queue         │
│ Status   │  │                         │     │               │
│          │  └─────────────────────────┘     │ Task          │
│          │                                  │ Queue         │
├──────────┴──────────────────────────────────┴───────────────┤
