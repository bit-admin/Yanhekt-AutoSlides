...
17. [共享工具模块](#17-共享工具模块)
18. [UI 架构](#18-ui-架构)
    - 18.1 [三面板布局](#181-三面板布局)
    - 18.2 [CSS 主题系统](#182-css-主题系统)
    - 18.3 [Z-index 层级](#183-z-index-层级)
    - 18.4 [共享样式类](#184-共享样式类)
19. [国际化](#19-国际化)
20. [构建系统](#20-构建系统)
    - 20.1 [Electron Forge + Vite](#201-electron-forge--vite)
    - 20.2 [Vite 配置](#202-vite-配置)
    - 20.3 [打包与分发](#203-打包与分发)
    - 20.4 [原生模块处理](#204-原生模块处理)
21. [依赖规则](#21-依赖规则)
    - 21.1 [主进程依赖图](#211-主进程依赖图)
    - 21.2 [渲染器依赖图](#212-渲染器依赖图)
    - 21.3 [ESLint 边界强制](#213-eslint-边界强制)
22. [IPC 通信全景](#22-ipc-通信全景)
23. [开发命令](#23-开发命令)
24. [验证步骤](#24-验证步骤)
25. [图像分析设计](#25-图像分析设计)
    - 25.1 [在线幻灯片检测](#251-在线幻灯片检测)
    - 25.2 [pHash 去重与排除](#252-phash-去重与排除)
    - 25.3 [AI / ML 语义过滤](#253-ai--ml-语义过滤)
    - 25.4 [自动裁剪](#254-自动裁剪)
...
- **云笔记**：集成 Yanhekt 云平台的富文本笔记，Editor.js 编辑器，支持分组、自动保存、实时搜索
...
│   │   ├── notesTypes.ts                     # 云笔记 API 类型 (NoteSummary, NoteDetail, NoteGroup 等)
│   │   └── index.ts                          # 导出聚合
│   │
│   ├── main/                                 # 主进程服务 (via @main/*)
│   │   ├── platform/                         # 平台基础设施
...
│   │   │   ├── notesService.ts               # Yanhekt 云笔记 API 客户端
│   │   │   └── campusNetworkProbe.ts         # 校园网在线状态探测
│   │   │
│   │   ├── infra/                            # 基础设施服务
...
│       │   │
│       │   └── cloudNotes/                   # 云笔记
│       │       └── useCloudNotes.ts          # 云笔记数据层 (分组/笔记/CRUD/过滤)
│       │
│       ├── components/                       # Vue 组件
│       │   ├── video/                        # 视频组件
...
| 领域 | 目录 | 职责 | 关键文件 |
|------|------|------|----------|
| **platform** | `src/main/platform/` | 平台基础设施 | `configService`, `authService`, `apiClient`, `intranetMappingService`, `windowManager`, `themeService`, `powerManagementService`, `cacheManagementService`, `notesService`, `campusNetworkProbe` |
...
`src/main/ipc/index.ts` 导出 `registerAllIpcHandlers(services)` 函数，内部调用 28 个领域 IPC 模块：
...
`src/main/ipc/types.ts` 定义 `IpcServices` 接口，包含 23 个服务实例：
...
`src/preload/index.ts` 组合 10 个领域模块：
...
| **cloudNotes** | `features/cloudNotes/` | 云笔记数据层 | `useCloudNotes` (分组/笔记/CRUD/客户端过滤+分页) |
...
├──────────────────────────┼──────────────────────────────────┤
│                    Preload (notes.ts)                         │
│                    cloudNotes namespace                       │
├──────────────────────────┼──────────────────────────────────┤
│                    Main Process                               │
│              notesIpc.ts → NotesService                       │
│                          │                                   │
│              cbiz.yanhekt.cn/v1/note*                         │
└─────────────────────────────────────────────────────────────┘
```

### 核心组件

**Composable** (`features/cloudNotes/useCloudNotes.ts`)：
- 数据层：管理分组（`groups`）、笔记列表（`notes`）、完整笔记集（`allNotes`）、选中笔记（`selectedNote`）
- 客户端过滤：`loadAll()` 一次加载完整笔记集（pageSize=500，最多 20 页），`applyView()` 根据分组 + 关键词过滤 + 分页
- CRUD 操作：创建、重命名、删除、移动分组、保存内容
- 原因：服务端 `/v1/note/list` 忽略 groupId 过滤，必须客户端过滤

**组件** (`components/cloudnotes/CloudNotesTab.vue`)：
- 三栏布局：分组侧栏（180px）+ 笔记列表（280px）+ Editor.js 编辑器
- Editor.js 工具：Header、List、Quote、Code、Table、Delimiter、Image
- 自动保存：`onChange` → 1s 防抖 → `flushSave()` 对比 block JSON 避免无操作网络请求
- 公共图片警告横幅（每会话可关闭）

**服务端** (`platform/notesService.ts`)：
- 运行在主进程，读取 electron-store 中的 auth token
- 绕过浏览器 CORS 限制（Node 无 CORS）
- API 基地址：`https://cbiz.yanhekt.cn`
- 认证头：`Xdomain-Client: web_user`（不需要 xclient 签名）

### 共享类型

`src/shared/notesTypes.ts` 定义跨进程类型：
...
export interface NoteDetail extends NoteSummary {
  content: string;  // Editor.js 文档，JSON 字符串
  // ...
}
...
| 通道 | 功能 |
|------|------|
| `cloudNotes:list` | 获取笔记列表（分页） |
| `cloudNotes:get` | 获取笔记详情 |
| `cloudNotes:create` | 创建空白笔记 |
| `cloudNotes:updateTitle` | 更新标题 |
| `cloudNotes:updateContent` | 更新内容 |
| `cloudNotes:moveToGroup` | 移动到分组 |
| `cloudNotes:delete` | 删除笔记 |
| `cloudNotes:groupList` | 获取分组列表 |
| `cloudNotes:groupCreate` | 创建分组 |
| `cloudNotes:groupDelete` | 删除分组（笔记移至默认组） |
| `cloudNotes:uploadImage` | 上传图片到 MinIO CDN |

### 关键设计决策

1. **客户端过滤**：服务端不支持 groupId 过滤，必须加载完整笔记集后客户端过滤
2. **自动保存**：Editor.js `onChange` → 1s 防抖 → `flushSave()` 对比 block JSON 避免无操作网络请求
3. **笔记切换**：切换笔记前先刷新当前笔记的待保存编辑
4. **分组删除**：服务端将分组内笔记移至默认组（id=0），客户端同步更新
5. **公共图片警告**：Yanhekt 将笔记图片存储在公共存储中，警告用户不要包含敏感图片
...
| `notesTypes.ts` | 云笔记 API 类型（`NoteSummary`、`NoteDetail`、`NoteGroup`、`NoteListResult`、`NotesResult<T>`、`NoteListParams`） |
...
const FEATURE_DOMAINS = ['video', 'results', 'offline', 'download', 'ai', 'export', 
                         'course', 'settings', 'platform', 'webCapture', 'tools', 'cloudNotes'];
