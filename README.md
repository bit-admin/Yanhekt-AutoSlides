...
> [!IMPORTANT]
> 🚧 **重大更新进行中** —— 本项目现正进行一项主要更新，并预计仍将持续一段时间。
>
> 下一个正式版本将是 **v5.0.0**，其中可能包含**大量破坏性变更**，涉及配置格式、数据存储结构及核心功能逻辑等多个方面。
>
> - 如果你是**新用户**，建议暂缓安装当前版本，等待 v5.0.0 正式发布后再开始使用，以避免后续迁移带来的困扰。
> - v5.0.0 **可能不会提供完整的向后兼容**，部分旧版本的配置或数据可能无法直接迁移。
> - 升级到 v5.0.0 时，**建议完全卸载旧版本并清除所有应用数据**（包括本地配置、缓存及用户数据目录）后重新安装，以确保应用正常运行。
>
> 我们会在版本发布前更新本说明。
...
- 在 `幻灯片审查` 页面中按课程文件夹审查提取结果，支持恢复、删除、自动裁剪、基准裁剪和裁剪状态回滚。
- 将提取后的幻灯片合并导出为 PDF 或 PPTX，并可通过 PNG 色彩压缩减小文件体积。
...
向导依次引导你设置：**输出目录**（幻灯片、下载与导出文件的保存位置）、**连接模式**（校园内网 / 外网）、**音频模式**（直播 / 录播是否静音）、**任务速度**（批量提取时的播放倍速）、**并行任务数**（同时处理的录播数量），以及可选的 **GitHub Copilot AI 过滤**（使用 GitHub 登录以启用免费的 gpt-4.1 模型来过滤非幻灯片画面）。
...
<p align="center">
  <img src="docs/onboarding-taskspeed.png" width="32%" alt="任务速度" />
  <img src="docs/onboarding-parallel.png" width="32%" alt="并行任务数" />
  <img src="docs/onboarding-ai.png" width="32%" alt="AI 过滤" />
</p>

完成配置后，向导的最后一步即是登录延河课堂。

### 5. 使用与设置

#### A. 登录

1. 你可以直接使用北京理工大学统一身份认证账户密码登录。账户密码登录有时会触发短信验证码二次验证，遇到这种情况请改用 `浏览器登录`。 [#1](https://github.com/bit-admin/Yanhekt-AutoSlides/issues/1)
2. 点击 `浏览器登录` 后，应用会在内置浏览器中打开官方登录界面。登录并跳转到延河课堂页面后，应用会自动获取延河课堂令牌。

<p align="center">
  <img src="docs/login.png" width="90%" alt="登录" />
</p>
...
所有设置集中在 `设置` 弹窗中（点击左下角用户栏的齿轮图标，或通过 `菜单栏 > AutoSlides / 文件 > 设置` 进入）。
...
<p align="center">
  <img src="docs/settings-general.png" width="42%" alt="一般设置" />
</p>
...
<p align="center">
  <img src="docs/settings-image-output.png" width="55%" alt="图像处理基础设置" />
</p>
...
应用采用三栏布局：左侧为导航边栏（`主页`、`直播`、`录播`，以及顶部的 `搜索` 框），中间为内容区（持久的 `课程信息` 浏览标签页 + 多个播放标签页），右侧为 `任务列表` / `下载队列` 切换面板。

1. `直播` 和 `录播` 两种模式可以同时运行；切换页面时，正在播放的视频不会被中断。
2. 点击 `录播` 或 `直播` 可进入对应的课程列表；或者使用顶部的 `搜索` 在全校课程中检索。你还可以观看任意正在直播的课程，即便你并未注册该课程。
3. 在 `主页` 的 `课程收藏夹` 中保存你常看课程的关键词，即可一键快速搜索。
...
1. 在 `录播` 网格中选择课程后，进入节次列表。可将具体节次添加到 `幻灯片提取任务列表`，也可下载 `课堂摄像头录像` 和 `屏幕录制` 视频。
...
处于播放模式时，可以同时观看 `屏幕录制` 和 `课堂摄像头` 视频；每个播放标签页相互独立，可在后台继续播放与提取。
...
3. 切换到 `屏幕录制` 单流视图后，播放页面下方会显示 `幻灯片提取预览画廊`；在 `播放模式` 下可以手动开启或停止 `幻灯片提取`，也可随时手动触发 `后处理` 或 `删除` 已提取的幻灯片。
...
当 `下载完成后自动使用 C++ 提取器提取幻灯片` 功能启用时，下载屏幕录制视频会在下载完成后自动运行 `幻灯片提取` 和 `后处理`；`下载队列` 中每个项目下方会显示提取与三阶段后处理的进度。
...
1. 点击左侧的 `工具` 按钮，进入 `幻灯片审查` 页面。
2. 审查页面分两级：先是按 `课程` 分组的文件夹列表，进入某个文件夹后查看其图像。
3. 如需删除文件夹，点击 `编辑` 按钮并选择目标文件夹。删除文件夹时，与该文件夹关联的回收站项目和裁剪前原始图像备份也会一并删除。

<p align="center">
  <img src="docs/results-folders.png" width="70%" alt="幻灯片审查 - 文件夹" />
</p>
...
<p align="center">
  <img src="docs/results-grid.png" width="70%" alt="幻灯片审查 - 图像网格" />
</p>
...
<p align="center">
  <img src="docs/pdfmaker.png" width="70%" alt="幻灯片导出" />
</p>
...
`设置` 弹窗分为五个标签页：`一般`、`图像处理`、`下载与播放`、`网络` 和 `AI`。

##### 设置 > 一般

1. 可调整主题、语言，或直接使用 `令牌` 登录。
...
1. 顶部 `下载完成后自动提取` 区域用于配置外部 `AutoSlides Extractor`。
2. `下载并发数`、`下载工作线程数` 和 `下载重试次数` 控制下载行为。
3. `显示更多播放速度` 控制播放模式下是否显示 0.5-16 倍速选项。
4. `阻止系统休眠` 可在视频播放期间阻止系统进入休眠状态，避免播放中断。
...
`内网网络接口` 可设置将内网模式流量绑定到指定的本地网络接口。此设置仅作用于经由内网模式的请求，其他流量仍然使用系统默认路由。`内网映射` 列出内网模式下使用的域名到 IP 的映射。
...
1. `讲座压缩` 可对下载的屏幕录制视频进行激进压缩，便于长期存档。它会降低帧率和分辨率，并可降低音频噪声，以较小文件体积保留基本可读性和相对清晰的音频。

<p align="center">
  <img src="docs/tools-compress.png" width="70%" alt="讲座压缩" />
</p>
...
<p align="center">
  <img src="docs/addons-yuketang.png" width="70%" alt="雨课堂扩展" />
</p>
...
<p align="center">
  <img src="docs/addons-webcapture.png" width="70%" alt="网页捕获扩展" />
</p>
