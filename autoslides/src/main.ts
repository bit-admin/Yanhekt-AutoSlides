...
import { pdfService, PdfMakeOptions, FolderEntry, CoverPageInfo } from './main/pdfService';
import {
  extractCourseName,
  extractSessionLabel,
  formatCoverTimestamp,
} from './main/coverFontService';
...
ipcMain.handle('intranet:getNetworkInterfaces', async () => {
  ...
  for (const [name, addrs] of Object.entries(raw)) {
    ...
    for (const addr of addrs) {
      ...
      if (addr.family !== 'IPv4') continue; // IPv4-only for intranet targets
      result.push({
        name,
...
ipcMain.handle('intranet:setInterfaceIp', async (_event, ip: string | null) => {
  ...
  if (normalized) {
    // Check whether the requested IP is currently present on any non-internal interface.
    const raw = os.networkInterfaces();
    let found = false;
...
// IPC handlers for lecture compression
...
// IPC handlers for offline processing
...
ipcMain.handle('pdfmaker:makePdf', async (_event, folders: FolderEntry[], requestOptions: PdfMakerRequestOptions) => {
  try {
    ...
    // Fallback to the English locale copy so we never have to maintain a
    // second hard-coded version of the copyright text in main process code.
    ...
    // Show save dialog. Default filename uses the first folder's course name,
    // matching what the cover (if any) will display.
