 ...
 * `name_zh`, on `Course.title` — and that value is what every filename, folder
 * name, `metadata.json`, Drive note title, and Index upload derives from. The
 * English name lives beside it on `titleEn` and must never be substituted into
 * those paths (see `@common/lectureNaming` and Recurring #1 in CLAUDE.md).
 *
...
export function academicTermLabel(
  ...
  const key = n === 1 ? 'searchPage.termRangeFall' : 'searchPage.termRangeSpring'
