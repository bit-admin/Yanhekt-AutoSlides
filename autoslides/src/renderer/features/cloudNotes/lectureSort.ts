/**
 * Natural ordering + course grouping for AutoSlides Index search results.
 *
 * Ported from the Index website's `share/apex/lectureSort.ts` (a separate
 * deployable that the app can't import from). Mirrors the app's recorded-folder
 * sort (`shared/utils/toolWindowFolders.ts`: week/weekday/period as a numeric
 * tuple, not lexicographic), extended with a school-year/semester tier so a
 * course shown across multiple terms lists the latest term first.
 */
