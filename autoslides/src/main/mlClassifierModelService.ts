...
const BUILTIN_MODEL_NAME = 'slide-classifier-v1.onnx';
const BUILTIN_MODEL_VERSION = 'slide-classifier-v1';
const CUSTOM_MODEL_FILENAME = 'custom-classifier.onnx';
const MAX_MODEL_BYTES = 200 * 1024 * 1024;

export class MlClassifierModelService {
  ...
  getActiveModelPath(): string {
    const cfg = this.config.getAIFilteringConfig();
    if (cfg.mlClassifierActiveModel === 'custom') {
      const customPath = this.getCustomModelPath();
      if (fs.existsSync(customPath)) return customPath;
      // Self-heal: custom file is gone, revert to builtin.
      this.config.setMlClassifierActiveModel('builtin');
      this.config.setMlClassifierCustomModelName(null);
    }
  ...
  getModelInfo(): MlClassifierModelInfo {
    const cfg = this.config.getAIFilteringConfig();
  ...
  async importCustomModel(srcPath: string): Promise<MlClassifierModelInfo> {
    ...
    this.config.setMlClassifierCustomModelName(path.basename(srcPath));
    this.config.setMlClassifierActiveModel('custom');
    this.cache = null;
    return this.getModelInfo();
  }

  async deleteCustomModel(): Promise<MlClassifierModelInfo> {
    const customPath = this.getCustomModelPath();
    try {
  ...
  async selectAndImportModel(): Promise<MlClassifierModelInfo | null> {
    const result = await dialog.showOpenDialog({
      title: 'Select Custom Classifier Model',
      properties: ['openFile'],
  ...
  async readModelBuffer(): Promise<ArrayBuffer> {
    ...
    if (!stat) {
      throw new Error(`ML classifier model not found at ${modelPath}`);
    }
