...
// Folder/image paths arrive from our own getFolders/getImages listings —
// they never contain `..` segments; reject any that do (defense in depth).
  ...
  ipcMain.handle('pdfmaker:makePdf', async (event, folders: FolderEntry[], requestOptions: PdfMakerRequestOptions) => {
    try {
      ...
      if (outputMode === 'batch') {
        ...
        const configuredOutputDir = expandTilde(configService.getConfig().outputDirectory);
        ...
        const outputDir = expandTilde(result.filePath);
